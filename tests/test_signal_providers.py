...
from gamepulse.database import DatabaseAssetError, connect_read_only, initialize_schema
...
class SourceNeutralProviderTests(unittest.TestCase):
    ...
    def test_lfs_pointer_database_reports_a_clear_asset_error(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            path = Path(temp_dir) / "gamepulse.sqlite3"
            path.write_text(
            ...
            with self.assertRaisesRegex(DatabaseAssetError, "Git LFS pointer"):
                connect_read_only(path)

    def test_invalid_database_returns_unavailable_instead_of_raising(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            path = Path(temp_dir) / "gamepulse.sqlite3"
            path.write_bytes(b"not a sqlite database")

            snapshot = SteamPublicGameSignalProvider(path).get_game_trends()

        self.assertEqual(snapshot.mode, "Unavailable")
        self.assertIn("not a valid SQLite database", snapshot.source_name)

    def test_composite_falls_back_to_snapshot_when_database_is_lfs_pointer(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            ...
            provider = CompositeSignalProvider(
                ...
                creator_directory_path=root / "missing.csv",
                snapshot_path=snapshot,
            )

            result = provider.get_game_trends()

        self.assertEqual(result.mode, "Demo")
        self.assertEqual(result.data[0].name, "Example")
