...
class DatabaseAssetError(sqlite3.DatabaseError):
    """Raised when the configured database asset is unavailable or invalid."""
...
def _validate_database_asset(database_path: Path) -> None:
    try:
        with database_path.open("rb") as handle:
            header = handle.read(max(len(_SQLITE_HEADER), len(_LFS_POINTER_HEADER)))
    except OSError as exc:
        raise DatabaseAssetError(
            f"Unable to read SQLite database asset: {database_path}"
        ) from exc

    if header.startswith(_LFS_POINTER_HEADER):
        raise DatabaseAssetError(
            f"SQLite database asset is a Git LFS pointer: {database_path}. "
            "Run 'git lfs pull' before launching GamePulse."
        )
    if not header.startswith(_SQLITE_HEADER):
        raise DatabaseAssetError(
            f"Database asset is not a valid SQLite database: {database_path}"
        )
...
def connect_read_only(database_path: Path) -> sqlite3.Connection:
    ...
    _validate_database_asset(database_path)
