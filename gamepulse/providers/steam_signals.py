...
from gamepulse.database import DatabaseAssetError, connect_read_only
...
class SteamPublicGameSignalProvider:
    ...
    def get_game_trends(self) -> SignalSnapshot[GameSignal]:
        ...
        except (DatabaseAssetError, sqlite3.DatabaseError, OSError) as exc:
            return SignalSnapshot(
                "Unavailable",
                "",
                f"GamePulse prepared Steam/public data unavailable: {exc}",
                [],
                "unavailable",
            )
        try:
