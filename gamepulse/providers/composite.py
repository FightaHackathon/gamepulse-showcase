...
class CompositeSignalProvider:
    ...
    def _twitch_games(self) -> SignalSnapshot[GameSignal] | None:
        ...
        data = [item if isinstance(item, GameSignal) else _legacy_twitch_game(item, snapshot) for item in snapshot.data]
        return SignalSnapshot(str(snapshot.mode), str(snapshot.observed_at), str(snapshot.source_name), data, "live" if str(snapshot.mode).casefold() == "live" else "fallback")
    ...
    def get_game_trends(self) -> SignalSnapshot[GameSignal]:
        ...
        return SignalSnapshot("Unavailable", "", "No game signal provider available", [], "unavailable")

    def _twitch_creators(self, game_id: str | None, game_name: str | None) -> list[CreatorSignal]:
        if self.twitch_provider is None:
            return []
        try:
            if hasattr(self.twitch_provider, "get_creators"):
                snapshot = self.twitch_provider.get_creators(game_id=game_id, game_name=game_name)
            ...
            return []
        return [item if isinstance(item, CreatorSignal) else _legacy_twitch_creator(item, snapshot) for item in snapshot.data]

    @staticmethod
    def _dedupe_creators(creators: Iterable[CreatorSignal]) -> list[CreatorSignal]:
        output: dict[tuple[str, str], CreatorSignal] = {}
        priority = {"live": 4, "creator submitted": 3, "imported": 2, "manual": 1, "demo": 0}
        for creator in creators:
            key = (creator.platform.casefold(), creator.creator_id.casefold())
            existing = output.get(key)
            if existing is None or priority.get(creator.source_mode.casefold(), 1) > priority.get(existing.source_mode.casefold(), 1):
                output[key] = creator
        return sorted(output.values(), key=lambda item: (item.name.casefold(), item.creator_id))
    ...
    def get_creators(
        ...
        if self.creator_provider_mode == "twitch":
            creators = self._twitch_creators(game_id, game_name)
        else:
            creators = list(self.creator_directory.get_creators(game_id, game_name).data)
            imported = self._imported()
            if imported is not None:
                for creator in imported.creators.data:
                    if game_name and creator.games and game_name.casefold() not in {name.casefold() for name in creator.games}:
                        continue
                    creators.append(creator)
            if self.creator_provider_mode == "auto":
        ...
        return SignalSnapshot(mode, observed_at, "Composite creator sources", creators, "mixed" if len(modes) > 1 else "provider")
