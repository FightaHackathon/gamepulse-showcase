...
Before workflow activation, preserve the model already selected for the chat. After explicit activation, use GPT-5.6 Sol as the only main orchestrator: medium reasoning for Medium and Heavy routes and light reasoning for the Light route. Do not use GPT-5.6 Luna, an executor, xhigh, or max as the orchestrator; Luna is reserved for subagents.

## Workflow Activation Gate

Custom workflow routing and agent delegation are opt-in per request. Do not spawn any custom agent or apply the Medium or Heavy route unless the user explicitly invokes codex-workflows:activate or explicitly tells Codex to activate codex-workflows. Do not carry activation into later requests. Without activation, use normal Codex behavior and do not delegate to these custom agents.
